...
public class ServerCommands {
    public static int share(ServerPlayer sender, ServerPlayer player, String waypointID) {
        try {
            ...
            Dispatcher.sendToClient(new JMWSActionPayload(CommandHelper.makeObjectShareRequestForUser(waypointStringJson, player.getUUID(), sender.getUUID(), ShareRequest.Direction.FOR_CLIENT)), player); // Send share request to player
            ...
            Dispatcher.sendToClient(new JMWSActionPayload(CommandHelper.makeObjectShareRequestForUser(waypointStringJson, player.getUUID(), sender.getUUID(), ShareRequest.Direction.FOR_HOST)), sender);
        } catch (Exception e)
        {
