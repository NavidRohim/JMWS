...
public class DatePicker extends Screen
{
    ...
    public void extractRenderState(@NonNull GuiGraphicsExtractor graphics, int mouseX, int mouseY, float a)
    {
        ...
        graphics.fill(gridLayout.getX(), gridLayout.getY(), gridLayout.getX() + gridLayout.getWidth(), gridLayout.getY() + gridLayout.getHeight() + 1, 0x80303030);

        // render the main layout
        super.extractRenderState(graphics, mouseX, mouseY, a);

        // render everything else (borders, text, background etc.)
        graphics.outline(gridLayout.getX(), gridLayout.getY(), gridLayout.getWidth(), gridLayout.getHeight() + 1, 0x40FFFFFF);
        graphics.centeredText(this.font, title, mainLayout.getX() + (widthHeightPicker / 2), mainLayout.getY() - minecraft.font.lineHeight - 2, -1);
    }

    private void render(boolean isInit)
    {
        this.clearWidgets();
        ...
        EditBox box = actionLayout.addChild(new EditBox(minecraft.font, 0, 0, widthHeightPicker, 15, Component.empty()));
    ...
    public LayoutElement setMonth(int monthIndex, int year)
    {
        ...
        gridLayout = new GridLayout(openAtX, openAtY);
        GridLayout.RowHelper helper = gridLayout.createRowHelper(5);

        Button leftButton = gridLayout.addChild(Button.builder(Component.literal("<"), button -> scrollLeft()).bounds(0, 0, dayWidthHeight, dayWidthHeight).build(), 0, 1, layoutSettings -> layoutSettings.padding(spacing).paddingVertical(1));
        Button leftButtonByYear = gridLayout.addChild(Button.builder(Component.literal("<<"), button -> scrollLeftByYear()).bounds(0, 0, dayWidthHeight, dayWidthHeight).build(), 0, 2, layoutSettings -> layoutSettings.padding(spacing).paddingVertical(1));
        gridLayout.addChild(Button.builder(Component.literal("OK"), button -> {calculateSelectionToMillis();}).bounds(0, 0, dayWidthHeight, dayWidthHeight).build(), 0, 3, layoutSettings -> layoutSettings.padding(spacing).paddingVertical(1));
        gridLayout.addChild(Button.builder(Component.literal(">>"), button -> scrollRightByYear()).bounds(0, 0, dayWidthHeight, dayWidthHeight).build(), 0, 4, layoutSettings -> layoutSettings.padding(spacing).paddingVertical(1));
        gridLayout.addChild(Button.builder(Component.literal(">"), button -> scrollRight()).bounds(0, 0, dayWidthHeight, dayWidthHeight).build(), 0, 5, layoutSettings -> layoutSettings.padding(spacing).paddingVertical(1));
